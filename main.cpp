...
/**
 * @function Print status of shared pointer on toy.
 * @param sharedToyPtr
 * @param tag
 */
void getSharedPtrStatus(SharedToyPtr* sharedToyPtr, std::string tag)
{
    std::cout << tag << " - control block address: " << sharedToyPtr->getControlBlock() << ", toy address: " << sharedToyPtr->get() << " links: " << sharedToyPtr->getUseCount() << "\n";
}
